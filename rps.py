    ...
    while True:
        try:
            ...
            if user_input in ('rock', 'paper', 'scissors', 'lizard', 'spock'):
                ...
                print("Nope! rock / paper / scissors / lizard / spock")
        except ValueError:
            print("Nope! rock / paper / scissors / lizard / spock")
    return user_input
...
class Player:
    ...
    def learn(self, move_self, their_move):
        return their_move
...
class Cycle(Player):
    def __init__(self):
        self.move_generator = self.cycle_moves()

    def move(self):
        return next(self.move_generator)

    def cycle_moves(self):
        i = 0
        while True:
            yield Player.moves[i]
            i = (i + 1) % len(Player.moves)
...
class Game:
    ...
    def play_round(self):
        ...
        time.sleep(0.2)
        print(f'{x.__class__.__name__} plays {move1}')
        time.sleep(speed)
        print(f'{y.__class__.__name__} plays {move2}')
        time.sleep(speed)
